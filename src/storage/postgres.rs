...
impl super::Storage for ClientGuard {
    ...
    async fn create_user(
        ...
        let stmt = db
            .prepare("INSERT INTO users(id, password_hash) VALUES ($1, $2);")
            .await?;
    ...
    async fn verify_password(&mut self, username: &str, password: &str) -> Result<bool, DbError> {
        let db = self.inner.as_mut().unwrap();
        let rows = db
            .query("SELECT password_hash FROM users WHERE id=$1;", &[&username])
            .await?;
    ...
    async fn delete_access_token(&mut self, token: uuid::Uuid) -> Result<(), DbError> {
        let db = self.inner.as_mut().unwrap();
        db.execute("DELETE FROM access_tokens WHERE token = $1;", &[&token])
            .await?;
        Ok(())
    }
    ...
    async fn get_profile(&mut self, username: &str) -> Result<Option<UserProfile>, DbError> {
        ...
        let rows = db
            .query(
                "SELECT avatar_url, display_name FROM users WHERE id = $1;",
                &[&username],
            )
            .await?;
    ...
    async fn add_pdus(&mut self, pdus: &[PduV4]) -> Result<(), DbError> {
        let db = self.inner.as_mut().unwrap();
        let stmt = db
            .prepare(
                "
    ...
    async fn get_events_since(&mut self, room_id: &str, since: u64) -> Result<Vec<Event>, DbError> {
        let db = self.inner.as_mut().unwrap();
    ...
    async fn get_event(&mut self, room_id: &str, event_id: &str) -> Result<Option<Event>, DbError> {
        let db = self.inner.as_mut().unwrap();
        let rows = db
            .query(
                "
        ...
        "m.room.member" => {
            ...
            if membership != "leave" {
                ...
                ",
                    ...
                    .await?;
                db.execute(&stmt, &[&event.state_key, &event.room_id])
                    .await?;
            }
        }
        "m.room.create" => {
